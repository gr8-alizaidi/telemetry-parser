...
export function renderStatusline(branch, counts, { initialized = true, pause = null } = {}) {
    ...
    return `${dot} rec ${sep} ${branch} ${sep} \x1b[2m${formatCounts(counts)}\x1b[0m`;
}
...
function cursorSessionId(input) {
    return typeof input.conversation_id === 'string' && input.conversation_id
...
export async function handleStatusline(ctx, input) {
    ...
    return renderStatusline(branch, counts);
}
...
async function main() {
    ...
    switch (subcommand) {
        ...
        case 'codex-user-prompt': await handleUserPrompt(ctx, input); return null;
